 ...
 *
 * Generation is done by looking up the translation for a specific key. The lookup order by locale
 * is:
 * - the locale provided in the method (optional)
 * - [locale]
 * - [Locale.ROOT]
 *
 * If no translation exists for that key, the return value is determined by the
 * [InvalidMessageProvider].
 *
 * Generation methods return one or more [Message] objects. To convert this to a single component,
 * the methods [lines] or [oneLine] can be used.
 *
...
interface Glossa {
   ...
   * @param args the arguments for templates in the translation, or [MessageArgs.empty] if not
   *   provided.
   */
  fun message(
      key: String,
      locale: Locale = this.locale,
      args: MessageArgs = MessageArgs.empty,
  ): Message
  ...
  /**
   * Creates a decorated instance which wraps around this instance, using a different default
   * locale.
   *
   * @param locale the new locale to translate with.
   */
  fun withLocale(locale: Locale): Glossa = GlossaWithLocale(this, locale)
}
...
data class MessageArgs(
  ...
  operator fun plus(other: MessageArgs) =
      MessageArgs(
